...
/**
 * LoginScreen.tsx — Google OAuth sign-in via Clerk
 * ─────────────────────────────────────────────────────────────────────────────
 * Single "Sign in with Google" button. After OAuth, Clerk sets isSignedIn=true,
 * App.tsx's useEffect fires, fetches the Supabase profile, and RootNavigator
 * automatically switches to the app — no navigation.navigate() needed here.
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
// Required by Clerk for OAuth redirects on Android
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (createdSessionId) {
        ...
        // Clerk is now signed in → App.tsx useEffect fires → RootNavigator switches
      }
    } catch (err: any) {
      ...
      <View style={s.container}>
        ...
        >
          {loading ? (
            ...
            <>
              <Ionicons name="logo-google" size={20} color={T.text} style={s.googleIcon} />
        ...
        <Text style={s.hint}>Use your DLSL school Google account (@dlsl.edu.ph)</Text>
...
const s = StyleSheet.create({
  ...
  googleIcon:    { },
  googleBtnText: { fontSize: 16, fontWeight: '600', color: T.text },
  ...
  errorBox: {
    ...
    borderRadius: 10, padding: 11, marginTop: 16,
  },
  errorText: { flex: 1, color: T.red, fontSize: 13, lineHeight: 18 },
