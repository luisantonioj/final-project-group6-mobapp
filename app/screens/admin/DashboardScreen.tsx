/**
 * admin/DashboardScreen.tsx — Admin control panel home
 * ─────────────────────────────────────────────────────────────────────────────
 * This is the DashboardTab for authenticated admins. Contains three panels
 * accessed via an internal tab bar (not the bottom tab bar):
 *
 *   ⏱ Countdown  — pause/resume timer, edit end time, toggle voting open
 *   📊 Results   — live leaderboard with position tabs, hide/show toggle
 *   📝 Posts     — full CRUD for announcements + polls, publish/draft
 *
 * CURRENT STATE:
 *   All data is served from DUMMY_* constants and a stub adminService object.
 *   The UI is fully built and functional with local state.
 *
 * BACKEND INTEGRATION — replace each stub in adminService:
 * ─────────────────────────────────────────────────────────────────────────────
 *   adminService.fetchPosts()       → usePosts() from app/hooks/usePosts.ts
 *   adminService.createPost()       → useCreatePost() mutation
 *   adminService.updatePost()       → useUpdatePost() mutation
 *   adminService.deletePost()       → useDeletePost() mutation (uses RPC)
 *   adminService.fetchSettings()    → useSettings() from app/hooks/useSettings.ts
 *   adminService.updateSettings()   → useUpdateSettings() mutation
 *   adminService.fetchCountdown()   → useSettings() (voting_start/end_time)
 *   adminService.fetchPositions()   → usePositions() from app/hooks/usePositions.ts
 *                                     with candidates joined
 *
 * REALTIME (add to useAdminStore useEffect):
 * ─────────────────────────────────────────────────────────────────────────────
 *   supabase.channel('admin-votes')
 *     .on('postgres_changes', { event: 'INSERT', table: 'Votes' }, () => {
 *       // re-fetch positions/vote counts
 *     })
 *     .subscribe();
 * ─────────────────────────────────────────────────────────────────────────────
 */
